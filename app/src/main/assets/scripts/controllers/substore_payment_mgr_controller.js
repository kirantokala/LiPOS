app.controller("SubstorePaymentMgrController", function ($scope, $http, $cookieStore, $rootScope, $mdDialog, $filter, $location, $window, commonService, UI_TRANSACTION, singleObjService) {
	$scope.user = $cookieStore.get("user");
	$scope.store = $cookieStore.get("store");
	
	$rootScope.pageTitle = "SUBSTORE PAYMENTS";
	...
	$scope.getInvestmentInfo = function(dateFrom, dateTo){
		...
		$http.get($rootScope.baseUrl+'action=getTransactionHomeData&date_from='+dateFrom+'&date_to='+dateTo+'&store_id='+$scope.store.storeId+'&user_id='+$scope.ownerType+'&role_id=8').success(function(data) {
	...
	$rootScope.getSubstorePaymentInfo = function(){
		$rootScope.loading = true;
		$http.get($rootScope.baseUrl+'action=getUserPaymentInfo&store_id='+$scope.store.storeId+"&role_id=8").success(function(data) {
    ...
    function infoController($rootScope, $scope,$mdDialog,singleObjService,commonService, UI_TRANSACTION){
		...
		$scope.acceptPayment = function(){
			$http({
	            url : $rootScope.baseUrl+'action=substorePaymentAccept&store_id='+$rootScope.store.storeId+'&user_id='+$scope.user.userId+'&status_id=2',
	            method : "POST",
