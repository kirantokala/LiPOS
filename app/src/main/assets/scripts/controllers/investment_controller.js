app.controller("InvestmentController", function ($scope, $http, $cookieStore, $rootScope, $mdDialog, $filter, $location, $window, commonService, UI_INVESTMENT, singleObjService) {
	$scope.user = $cookieStore.get("user");
	$scope.store = $cookieStore.get("store");
	
	$rootScope.pageTitle = "INVESTMENT";
	
	$scope.UI_INVESTMENT = UI_INVESTMENT;	
	...
	$scope.getInvestmentInfo = function(dateFrom, dateTo){
		...
		$http.get($rootScope.baseUrl+'action=getTransactionHomeData&date_from='+dateFrom+'&date_to='+dateTo+'&store_id='+$scope.store.storeId+'&user_id='+$scope.ownerType+"&role_id=1").success(function(data) {
			$scope.infos = data.result;
		    $rootScope.loading = false;
		});
		$http.get($rootScope.baseUrl+'action=getUserInvestInfo&date_from='+dateFrom+'&date_to='+dateTo+'&store_id='+$scope.store.storeId+'&user_id='+$scope.ownerType).success(function(data) {
			$scope.userInvInfos = data.result;
		    $rootScope.loading = false;
		});
	}
	...
	$scope.openAddInvestDialog = function(ev) {
        ...
        $mdDialog.show({
          ...
          templateUrl: 'templates/dialogs/investment_add.html',
    ...
    $scope.openInvestInfoDialog = function(ev,info) {
        ...
        $mdDialog.show({
          ...
          templateUrl: 'templates/dialogs/investment_info.html',
          parent: angular.element(document.body),
          targetEvent: ev,
          clickOutsideToClose:true,
          fullscreen: $scope.customFullscreen // Only for -xs, -sm breakpoints.
    ...
    function addController($rootScope, $scope,$mdDialog,singleObjService,commonService, UI_INVESTMENT){
    	$scope.ADD_INVESTMENT = UI_INVESTMENT.add_investment;
		var status = {};
		status.id = 1;
    ...
    function infoController($rootScope, $scope,$mdDialog,singleObjService,commonService, UI_INVESTMENT){
    	$scope.INVESTMENT_INFO = UI_INVESTMENT.investment_info;
    	$scope.info = singleObjService.getObj();
		$scope.user = $rootScope.user;
		
		$scope.acceptPayment = function(){
			$http({
	            url : $rootScope.baseUrl+'action=investmentAccept&store_id='+$rootScope.store.storeId+'&user_id='+$scope.user.userId+'&status_id=2',
	            method : "POST",
