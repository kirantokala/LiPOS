app.controller("SubstoreOrderController", function ($scope, $http, $mdDialog, $cookieStore, $filter, $rootScope, commonService, singleObjService, UI_SUBSTORE_ORDERS) {
	...
	$scope.getCredits = function(dateFrom, dateTo){
		...
		$http.get($rootScope.baseUrl+'action=getSubstoreOrders&date_from='+dateFrom+'&date_to='+dateTo+'&store_id='+$scope.store.storeId+'&user_id='+$scope.user.userId).success(function(data) {
			
		      $scope.substoreOrders = data.result;
    ...
    function itOrdController($scope,$mdDialog,singleObjService, UI_SUBSTORE_ORDERS){
