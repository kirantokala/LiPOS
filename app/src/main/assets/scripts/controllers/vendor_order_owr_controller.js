app.controller("VendorOrderOwrController", function ($scope, $http, $mdDialog, $cookieStore, $filter, $rootScope, commonService, singleObjService, UI_VENDOR) {
	$scope.user = $cookieStore.get("user");
	
	$rootScope.pageTitle = "VENDOR ORDERS";
	...
	$scope.getCredits = function(dateFrom, dateTo){
		...
		$http.get($rootScope.baseUrl+'action=getStoreOrders&date_from='+dateFrom+'&date_to='+dateTo+'&store_id='+$scope.store.storeId+'&user_id=0&vendor_id=-1').success(function(data) {
    ...
    function itOrdController($scope, $rootScope, $mdDialog, singleObjService, UI_VENDOR){
    	...
    	$scope.acceptOrder = function(statusId){
  	    ...
  	    $scope.answer = function(statusId) {
  	    	...
  	    	//$scope.order.paymentBill = $scope.paymentBill;
