app.controller("VendorPaymentStoreController", function ($scope, $http, $cookieStore, $rootScope, $mdDialog, $filter, $location, $window, commonService, UI_TRANSACTION, singleObjService) {
	$scope.user = $cookieStore.get("user");
	$scope.store = $cookieStore.get("store");
	
	$rootScope.pageTitle = "VENDOR PAYMENTS";
	...
	$scope.getInvestmentInfo = function(dateFrom, dateTo){
		...
		$http.get($rootScope.baseUrl+'action=getVendorTransactionHomeData&date_from='+dateFrom+'&date_to='+dateTo+'&store_id='+$scope.store.storeId+'&user_id='+$scope.ownerType+"&role_id=9").success(function(data) {
	...
	$scope.getSubstorePaymentInfo = function(){
		$rootScope.loading = true;
		$http.get($rootScope.baseUrl+'action=getUserPaymentInfo&store_id='+$scope.store.storeId+"&role_id=9").success(function(data) {
